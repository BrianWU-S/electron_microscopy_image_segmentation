...
# tf.config.experimental_run_functions_eagerly(True)        # run eagerly can slow down the training speed
...
def visualize_training_results(hist, save_path="../results/UNet/Unet_training", loss_flag=True, acc_flag=True,
                               lr_flag=False, bce_flag=True, bdl_flag=True, iou_flag=True):
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    ...
    bdl = hist.history['bce_dice_loss']
    iou = hist.history['mean_io_u']
    if loss_flag:
    ...
    if lr_flag:
        ...
        plt.show()
    if bce_flag:
        plt.plot(np.arange(len(bce)), bce)
        plt.scatter(np.arange(len(bce)), bce, c='g')
        plt.xlabel("Epoch")
        plt.ylabel("Binary crossentropy loss")
        plt.title("Training BCE loss")
        plt.savefig(os.path.join(save_path, "bce.png"))
        plt.show()
    if bdl_flag:
        plt.plot(np.arange(len(bdl)), bdl)
        plt.scatter(np.arange(len(bdl)), bdl, c='g')
        plt.xlabel("Epoch")
        plt.ylabel("Binary Dice Loss")
        plt.title("Training Binary Dice Loss")
        plt.savefig(os.path.join(save_path, "bdl.png"))
        plt.show()
    if iou_flag:
        plt.plot(np.arange(len(iou)), iou)
        plt.scatter(np.arange(len(iou)), iou, c='g')
        plt.xlabel("Epoch")
        plt.ylabel("Mean IOU")
        plt.title("Training Mean-IOU")
        plt.savefig(os.path.join(save_path, "iou.png"))
        plt.show()
...
def dice_coef(y_true, y_pred):
    ...
    smooth = 1.
    y_true_f = K.flatten(y_true)  # (None,)
    y_pred_f = K.flatten(y_pred)  # (None,)
...
def weighted_BCE_loss(y_true, y_pred, positive_weight=3):
...
def mean_iou(y_true, y_pred):
    """ this function has many problems, just replace it by: tf.keras.metrics.MeanIoU(num_classes=2)"""
    prec = []
