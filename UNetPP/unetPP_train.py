...
if __name__ == '__main__':
    ...
    myGenerator = trainGenerator(batch_size=BATCH_SIZE, train_path='../dataset',
                                 ...
                                 aug_dict=data_gen_args_3, image_color_mode="grayscale", mask_color_mode="grayscale",
    ...
    model = UNetPlusPlus(deep_supervision=False, pretrained_weights=None, input_size=(512, 512, 1), lr=LR, num_class=1,
                         bn_axis=3)
    callbacks_list = [
        ModelCheckpoint('../models/UNetPP/unetPP_membrane_bestIOU.hdf5', monitor='mean_io_u', mode='max', verbose=1, save_best_only=True),
        ModelCheckpoint('../models/UNetPP/unetPP_membrane_bestloss.hdf5', monitor='loss', verbose=1,
                        save_best_only=True),
        ModelCheckpoint('../models/UNetPP/unetPP_membrane_bestacc.hdf5', monitor='accuracy', mode='max',verbose=1,
                        save_best_only=True),
    ...
    visualize_training_results(hist=hist, save_path="../results/UNetPP/UnetPP_training", loss_flag=True, acc_flag=True,
                               lr_flag=True)
    ...
    model.save(filepath=r"../models/UNetPP/unetPP_e%i_s_%i_lr_%f.hdf5" % (EPOCHES, STEPS, LR))
